...
if len(sys.argv) > 1:
    if len(sys.argv) > 2:
        if sys.argv[1] == 'findid':
            ...
            city = sys.argv[1:]
            city = '%20'.join(city)
            apiurl = f'http://api.openweathermap.org/data/2.5/weather?q={city}&appid={key}&units=metric'
    
    else:
        city = sys.argv[1]
        apiurl = f'http://api.openweathermap.org/data/2.5/weather?q={city}&appid={key}&units=metric'
else:
    apiurl = f'http://api.openweathermap.org/data/2.5/weather?q=montreal,ca&appid={key}&units=metric'
